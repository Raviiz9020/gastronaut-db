...
export function VendorSpecialBanner({ special, onResetCategory }: VendorSpecialBannerProps) {
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden rounded-xl p-2.5 sm:p-3 mb-3.5 border shadow-2xs bg-gradient-to-r",
        theme.bannerGradient
      )}
    >
      <div className="flex items-center justify-between gap-2.5">
        {/* Left: Icon + Title & Info inline */}
        <div className="flex items-center gap-2.5 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1 flex items-center gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <span
                className={cn(
                  "inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-wider border",
                  theme.badgeBg
                )}
              >
                <span className={cn("w-1 h-1 rounded-full animate-pulse", theme.dotColor)} />
                Chef&apos;s Special
              </span>
              <span className="text-[10px] sm:text-xs font-semibold text-muted-foreground">
                ({special.itemCount} {special.itemCount === 1 ? 'Dish' : 'Dishes'})
              </span>
            </div>
          </div>
        </div>

        {/* Right Action Button: Clear filter */}
        <button
          type="button"
          onClick={onResetCategory}
          className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] sm:text-xs font-bold bg-background/80 hover:bg-background border border-border/80 shadow-2xs hover:border-foreground/20 text-foreground transition-all cursor-pointer shrink-0"
        >
          <span>View All</span>
          <X className="h-3 w-3" />
        </button>
